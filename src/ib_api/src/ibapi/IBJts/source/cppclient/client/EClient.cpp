...
bool EClient::useProtoBuf(int msgId) {
    return serverVersion() >= MIN_SERVER_VER_PROTOBUF && PROTOBUF_MSG_IDS.find(msgId) != PROTOBUF_MSG_IDS.end();
}
...
void EClient::reqExecutionsProtoBuf(int reqId, const ExecutionFilter& filter)
{
    // not connected?
    if (!isConnected()) {
        m_pEWrapper->error(NO_VALID_ID, Utils::currentTimeMillis(), NOT_CONNECTED.code(), NOT_CONNECTED.msg(), "");
    ...
    try {
        ...
        if (filter.m_clientId != UNSET_LONG) {
            executionFilter.set_clientid(filter.m_clientId);
        }
        if (!filter.m_acctCode.empty()) {
            executionFilter.set_acctcode(filter.m_acctCode.c_str());
        }
        if (!filter.m_time.empty()) {
            executionFilter.set_time(filter.m_time.c_str());
        }
        if (!filter.m_symbol.empty()) {
            executionFilter.set_symbol(filter.m_symbol.c_str());
        }
        if (!filter.m_secType.empty()) {
            executionFilter.set_sectype(filter.m_secType.c_str());
        }
        if (!filter.m_exchange.empty()) {
            executionFilter.set_exchange(filter.m_exchange.c_str());
        }
        if (!filter.m_side.empty()) {
            executionFilter.set_side(filter.m_side.c_str());
        }
        if (filter.m_lastNDays != UNSET_INTEGER) {
            executionFilter.set_lastndays(filter.m_lastNDays);
        }
        
        if (!filter.m_specificDates.empty()) {
            for (long specificDate : filter.m_specificDates) {
                executionFilter.add_specificdates(specificDate);
            }
        }

        // execution request
        protobuf::ExecutionRequest executionRequest;
        executionRequest.set_reqid(reqId);
        executionRequest.set_allocated_executionfilter(&executionFilter);

        // send execution request
        ENCODE_MSG_ID(REQ_EXECUTIONS + PROTOBUF_MSG_ID);
        executionRequest.SerializeToOstream(&msg);

        executionRequest.release_executionfilter();
    }
    catch (EClientException& ex) {
        m_pEWrapper->error(reqId, Utils::currentTimeMillis(), ex.error().code(), ex.error().msg() + ex.text(), "");
        return;
...
void EClient::reqExecutionsNonProtoBuf(int reqId, const ExecutionFilter& filter)
{
