...
public abstract class EClient {
    ...
    public static final int MAX_VERSION = MIN_SERVER_VER_ZERO_STRIKE; // ditto
    
    public static final int PROTOBUF_MSG_ID = 200;
    public static final List<Integer> PROTOBUF_MSG_IDS = Arrays.asList( REQ_EXECUTIONS );
    ...
    private boolean useProtoBuf(int msgId) {
        return serverVersion() >= MIN_SERVER_VER_PROTOBUF && PROTOBUF_MSG_IDS.contains(msgId);
    }
    ...
    private synchronized void reqExecutionsProtoBuf(int reqId, ExecutionFilter filter) {
        // not connected?
        if( !isConnected()) {
            notConnected();
            return;
        ...
        try {
            ...
            if (Util.isValidValue(filter.clientId())) {
                executionFilterBuilder.setClientId(filter.clientId());
            }
            if (!Util.StringIsEmpty(filter.acctCode())) {
                executionFilterBuilder.setAcctCode(filter.acctCode());
            }
            if (!Util.StringIsEmpty(filter.time())) {
                executionFilterBuilder.setTime(filter.time());
            }
            if (!Util.StringIsEmpty(filter.symbol())) {
                executionFilterBuilder.setSymbol(filter.symbol());
            }
            if (!Util.StringIsEmpty(filter.secType())) {
                executionFilterBuilder.setSecType(filter.secType());
            }
            if (!Util.StringIsEmpty(filter.exchange())) {
                executionFilterBuilder.setExchange(filter.exchange());
            }
            if (!Util.StringIsEmpty(filter.side())) {
                executionFilterBuilder.setSide(filter.side());
            }
            if (Util.isValidValue(filter.lastNDays())) {
                executionFilterBuilder.setLastNDays(filter.lastNDays());
            }
            if (filter.specificDates() != null) {
                filter.specificDates().stream().forEach(specificDate -> executionFilterBuilder.addSpecificDates(specificDate));
            }

            // execution request
            ExecutionRequestProto.ExecutionRequest.Builder executionRequestBuilder = ExecutionRequestProto.ExecutionRequest.newBuilder();
            if (Util.isValidValue(reqId)) {
                executionRequestBuilder.setReqId(reqId);
            }
            executionRequestBuilder.setExecutionFilter(executionFilterBuilder.build());
            ExecutionRequestProto.ExecutionRequest executionRequest = executionRequestBuilder.build();

            // send execution request
            byte[] byteArray = executionRequest.toByteArray();
    ...
    private synchronized void reqExecutionsNonProtoBuf(int reqId, ExecutionFilter filter) {
        // not connected?
        if( !isConnected()) {
            notConnected();
            return;
