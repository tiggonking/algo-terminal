...
class EClient(object):
    ...
    def useProtoBuf(self, msgId: int) -> bool:
        return self.serverVersion() >= MIN_SERVER_VER_PROTOBUF and msgId in PROTOBUF_MSG_IDS
    ...
    def reqExecutions(self, reqId: int, execFilter: ExecutionFilter):
        ...
        if (self.useProtoBuf(OUT.REQ_EXECUTIONS)):
            self.reqExecutionsProtoBuf(reqId, execFilter);
        else:
            self.reqExecutionsNonProtoBuf(reqId, execFilter);
    ...
    def reqExecutionsProtoBuf(self, reqId: int, execFilter: ExecutionFilter):
        ...
        if execFilter.clientId != UNSET_INTEGER:
